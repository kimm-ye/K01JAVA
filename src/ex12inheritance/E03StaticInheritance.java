...
public class E03StaticInheritance {
	...
	public static void main(String[] args) {
		...
		/*
		멤버변수가 static이기 때문에 값이 하나라서
		
		멤버변수 val은 static이므로 프로그램 시작시 하나만 생성되어
		메서드영역에 로드된다.
		따라서 아래에서의 모든 연산은 val에 모두 더해지게 된다.
		 */
		Adder ad = new Adder();
		AdderFriend adFriend = new AdderFriend();
		
		ad.add(1); 				//1+1=2
		adFriend.frendAdd(3);   //2+3+3 =8
		Adder.val += 5; 		//8+5 =13
