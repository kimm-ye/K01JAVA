...
public class E10MyFriendInfoBook2 {
	...
	public static void main(String[] args) {
		...
		FriendInfoHandler2 handler = new FriendInfoHandler2(100); //num = 100 즉, 100명을 저장할 수 있는 크기
...
class FriendInfoHandler2{
	...
	private Friend2[] myFriends; //프렌드 타입의 객체 배열 -> 객체를 저장할 수 있는 배열
	//실제 저장된 연락처 갯수
	...
	public FriendInfoHandler2(int num) {
		//num 크기의 객체배열 생성
		myFriends = new Friend2[num]; //해당 객체 생성. num을 가지고 객체 크기 파악
	...
	public void addFriend(int choice) {
		...
		if(choice==1) { //고딩친구 입력 후 추가
			...
			HighFriend2 high = new HighFriend2(iName, iPhone, iAddr, iNickname); //결과는 동일하나 작성방법만 다름 . 방법1 객체생성 후 후위연산통해서
			myFriends[numOfFriends++] = high; //후위연산
		}
		else if(choice==2) { //대딩친구 입력 후 추가
			System.out.print("전공:"); iMajor = scan.nextLine();
			myFriends[numOfFriends++] = new UnivFriend2(iName, iPhone, iAddr, iMajor);//결과는 동일하나 작성방법만 다름 . 객체를 생성함과 동시에 집어넣음
		}
	...
	public void searchInfo() {
		...
		for(int i=0; i<numOfFriends; i++) {
			...
			if(serachName.compareTo(myFriends[i].name)==0) {
				...
				//만약에 여기서 boolean없이 바로 return때려버리면 반복안하고 바로 찾는 정보 없다고 결과 나오고 호출한 곳으로 넘어가버림
			}
		}
	...
	public void deleteInfo() {
		...
		인덱스는 0부터니까
		 */
		int deleteIndex =-1; //인덱스는 0부터 시작인데 -1을 뱉는다는건 일치하는 값이 없다는 뜻
		...
		for(int i=0; i<numOfFriends; i++) {
			...
			if(deleteName.compareTo(myFriends[i].name)==0) {
				...
				//삭제한 요소의 index를 저장한다.
				deleteIndex = i; //i를 deleteIndex에 담는다. i번째를 삭제했다는 뜻 
		...
		if(deleteIndex == -1) {
			...
			for(int i=deleteIndex; i<numOfFriends; i++) { //null값으로 구멍난 곳 메꾸는 작업
				myFriends[i] = myFriends[i+1]; //앞으로 땡기는 거임 안하면 nullException뜸 즉, i+1에 잇는 걸 i로 대입하는거지
			}
