...
class C extends B{
	...
	모두 가려지게 되므로 C클래스에 정의된 메서드 하나만 남게 된다/
	오버로딩된 메서드는 서로 다르므로 3개모두 존재하게 된다.
	 */
}
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		즉 아래부분은 모두 C클래스에 생성된 rideMethod()
		메소드가 호출된다.
		 */
	...
	/////////////////////////////////////////////////////////////////////////////
		...
		//부모 참조 변수로 자식 객체를 참조할 수 있다.
		A refNum1 = new B(); //가능
		A refNum2 = new C(); //가능
		B refNum3 = new C(); // C한테 B부모 가능
		 
		C refAddr1 = new C(); // C가 C니까 가능
		B refAddr2 = refAddr1; //B타입 => C타입 참조하므로 가능
		A refAddr3 = refAddr1; //A타입 => C타입 참조하므로 가능
		
		/*
		참조변수 refId1이 C객체를 참조하고 있으므로 마치 refId1이 C타입이라고
		착각할 수 있으나, 처음부터 refId1은 A타입의 참조 변수이다.
		자식으로 부모를 참조하는 것은 불가능하므로 아래 2개의 문장은 
		에러가 발생한다.
		 */
		A refId1 = new C(); //A타입 => C타입 참조하므로 가능
		//B refId2 = refId1; //B타입 -> A타입 참조 불가 (자식이 부모 참조불가) //에러발생
		//C refId3 = refId1; //C타입 -> A타입 참조 불가(자식이 부모 참조 불가) //에러발생
