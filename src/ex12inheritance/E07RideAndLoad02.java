...
public class E07RideAndLoad02 {
	...
	public static void main(String[] args) {
		...
		/*
		오버라이딩 처리된 메서드이므로 참조변수에 상관없이
		자식쪽 메서드가 호출된다. 참조 변수의 영향을 받지 않는다.
		 */
		parent.excecise();
		parent.sleep();
		parent.printParent();
		
		//parent.study(); //자식에서 확장된 메서드이므로 접근불가. 에러발생
		parent.walk();
		//parent.walk(20); //자식에서 확장된 메서드이므로 접근불가. 에러발생
		
		// 정적메서드는 오버라이딩 되지 않고, 클래스명으로 호출한다.
		//child는 child, parent는 parent
