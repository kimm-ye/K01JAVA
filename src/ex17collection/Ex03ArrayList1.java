...
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		...
		/*
		1-1] 객체저장
			add(객체) : 순자척으로 저장하면서 인덱스는 0부터 자동으로 부여된다.
		...
		list.add("트와이스");
		list.add(3, "워너원");
		//list.add(5, "소방차ㅋㅋ"); // 에러발생. 인덱스를 건너뛸수없다. // 4번째가 나와야하는데 5번째를 기재해서 오류
		list.add(list.size(), "오마이걸"); //length 는 배열의 전체 크기 / size는 리스트에 들어있는 원소의 갯수
		...
		2. hasNext()로 반환할 객체가 있는지 확인하고, next()로 객체를 반환한다. //hasNext로 값이 있는지 확인하는 거지 //출력할때는 next로 꺼내고 반복
		...
		방법1 : 인덱스로 삭제한다. 삭제가 완료되면 삭제된 객체를 반환한다.
			...
			삭제 성공시 해당 객체를 반환하므로 어떤 객체를 삭제했는지 확인할 수 있다.
		*/
		Object obj = list.remove(2);
		System.out.println("삭제된 객체:" + obj); //toString 객체때문에 바로 확인가능
		...
		즉, "방탄소년단" 이게 참조값임. 스트링X
		스트링은 자주 써서 "방탄소년단".length()바로 할 수 있음
		 */
		...
		/*
		 1-8] 전체삭제 : 두가지 메서드를 통해 전체삭제 할 수 있다.
		 */
