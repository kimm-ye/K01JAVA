...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		...
		/*
		아래 2개의 컬렉션은 내부적으로 자료구조만 다를뿐, 구현방법은 완전히 동일하다.
		...
		//컬렉션에서 인스턴스 저장
		//list는 인덱스가 생성되기 떄문에 중복저장이 가능하다.
		list2.add(st1); //인덱스0
		list2.add(st2);
		list2.add(st3);
		list2.add(st4); //인덱스3
		...
		for(Student st : list2) { //순서가 있기 때문에 제일 앞에거가 삭제됨
			st.showInfo(); //4개출력
		}
		
		System.out.println("[Iterator(이터레이터) 사용]");
		Iterator<Student> it2 = list2.iterator();
		while(it2.hasNext()) {
//			Student student = it2.next();
//			student.showInfo();
			//위 2줄의 코드를 아래 1줄로 고칠수 있다.
			it2.next().showInfo(); //위에 두 문장과 동일하다
		}
		
		//List계열의 컬렉션은 인덱스로 접근할수있으므로 get메소드 사용해야함
		System.out.println("[일반 for문 사용]");
		for(int i=0; i<list2.size(); i++) {
			list2.get(i).showInfo(); //get메소드
		}
		...
		외부에서는 직접 접근할 수 없으므로	getter/setter 를 통해 접근해야한다.
		remove()메서드는 삭제에 성공할 경우 삭제된 객체를 반환하므로
		삭제된 객체의 정보를 출력할 수 있다.
		 */
		//인덱스를 통한 삭제(객체 자체가 반환됨)
		System.out.println("삭제된 객체의 이름:" + list2.remove(2).getName()); //getter
		
		//인스턴스 참조값을 통한 삭제(boolean값이 반환됨)
		System.out.println("인스턴스를 통한 삭제:" + list2.remove(st1)); //arraylist1에서는 "방탄소년단"으로 인스턴스 참조값 넣었고 현재 여기는 st1, st2로 넣었기에
