...
class YesThread extends Thread{
	 ...
	 */
	public YesThread() {} //기본생성자 이름 자동으로 부여됨
	
	/*
	Thread클래스의 생성자를 호출할때 전달하는 매개변수를 통해 해당 쓰레드의 이름을 지정할 수 있다.
	지정된 이름은 getName()을 통해 반환할 수 있다.
	 */
	public YesThread(String threadName) {
		super(threadName); //super를 통해서 이름 지정 가능
	}
	void threadMethod() throws InterruptedException{
		for(int i=1; i<=10; i++) {
			//반복시의 횟수와 스레드의 이름을 출력한다.
			System.out.println(String.format("%s]i=%d", getName(), i));
			//2초마다 블럭상태로 들어간다.
			sleep(2000); //여기 시간을 짧게하면 독립쓰레드가 빨리 끝나버리니까 밑에 진행되는 데몬쓰레드가 몇번 출력 안되고 끝남
		}
	}
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		...
		yt2.setPriority(Thread.MAX_PRIORITY); //우선순위는 10으로 지정됨 (MAX니까)
		yt2.start();
		...
		dt.setName("난 데몬쓰레드");
		dt.setDaemon(true); //setDaemon이 있으면 데몬쓰레드 없으면 독립쓰레드 // setDaemon은 독립쓰레드를 종속쓰레드로 만들어준다.
		dt.start();
		
		//Thread클래스의 정적메서드를 통해 여러가지 정보확인
		System.out.println("현재활성화된 상태의 쓰레드수:" + Thread.activeCount());
		System.out.println("현재활성화된 쓰레드명:" + Thread.currentThread().getName());  //실행하는 Thread 인스턴스의 레퍼런스를 반환
		System.out.println("메인함수의 우선순위:" + Thread.currentThread().getPriority()); //쓰레드 우선순위 반환
