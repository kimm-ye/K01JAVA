...
//		MessageSendingThread tr3 = new MessageSendingThread("세번째");
		...
		2단계 실행
			...
			우선순위는 1~10사이에서 부여할 수 있고, 높을 수록 우선순위도 높아지게 된다.
			
			※단, 우선순위가 높다고 무조건 먼저 실행되지는 않는다.
			전체적인 확률이 높아지게 되어 먼저 실행될 가능성이 높아진다고 생각하면 된다.
		 */
//		MessageSendingThread tr1 = new MessageSendingThread("첫번쨰", 1);
//		MessageSendingThread tr2 = new MessageSendingThread("두번쨰", 10);
//		MessageSendingThread tr3 = new MessageSendingThread("세번쨰", 5);
		...
		3단계 실행
			...
			결과는 2단계와 동일하다.
		 */
		MessageSendingThread tr1 = new MessageSendingThread("첫번째", Thread.MAX_PRIORITY); //static형 상수라서 글자 기울어짐
