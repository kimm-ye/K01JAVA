...
public class E07ForEachBasic {
	...
	public static void main(String[] args) {
		...
		/*
		배열 arr의 요소를 하나씩 참조하여 출력하고, 더 이상 참조할 요소가 없을때
		for문이 자동으로 종료된다.
		 */
		sum = 0;
		for(int e : arr) {
		...
		for(int b : arr) {
			b++;
			System.out.print(b+" "); //1 증가된 값 출력됨.
		}
		System.out.println();
		...
		/*
		2차원배열은 1차원배열을 겹쳐놓은 형태이므로 우선 하나의 행을 읽어와서
		각 요소를 하나씩 참조하여 출력한다.
		 */
		...
		for(int[] a : twoDim) { //한 행을 읽어와서
			for(int b : a) { //1차원배열접근과 동일하게 요소를 참조하여 출력한다.
				System.out.print(b+ " ");
			}
			System.out.println();
		}
