...
public class E02StringMethod {
	...
	public static void main(String[] args) {
		...
		/*
		 7] endWith()
		 	: 특정문자열로 끝날때 true를 반환한다.
		 8] startsWith()
		 	: 특정문자열로 시작할때 true를 반환한다.
		 */
		...
		13] split()
			...
			해당 구분자가 업는 경우에는 크기가 1인 배열로 반환된다.
		 */
		...
		14] substring()
			...
			사용1: substring(인덱스) => 이경우 인덱스로부터 끝까지를 잘라낸다
			사용2: substring(시작인덱스, 끝인덱스) => 인덱스에 해당하는 부분만 잘라서 반환한다.
		 */
		...
		//파일명의 뒷부분 부터 .의 위치를 찾는다
		int beginIndex = filename.lastIndexOf(".") + 1;
		System.out.println("파일의 확장자는:" + filename.substring(beginIndex));
		
		/*
		 15] codePointAt(인덱스)
		 	: 문자열의 특정 인덱스의 문자를 찾은 후 아스키 코드(유니코드)를 반환한다.
		 */
		System.out.println("### codePointAt() ###");
		System.out.println("str1 첫번째 문자의 아스키코드:" + str1.codePointAt(0)); //W 아스키코드
		System.out.println("str2 첫번째 문자의 아스키코드:" + str2.codePointAt(0)); //자 아스키코드
	}
}
