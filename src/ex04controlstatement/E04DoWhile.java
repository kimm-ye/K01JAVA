...
public class E04DoWhile {
	...
	public static void main(String[] args) throws IOException{
		...
		/*
		시나리오] E03 예제의 1~10까지 더하는 수열문자를 do~while문으로 
			변경하여 구현하시오.
		 */
		int sum = 0; //합의 초기화
		int i = 1; //반복을 위한 변수
		...
		do { //조건검사 없이 진입
			if (j%4 == 0 || j%7 == 0) { //4 또는 7의 배수일때..
				//System.out.println("j:" + j); //배수확인용 출력
		...
		Scanner scanner = new Scanner(System.in); //Scanner를 쓰거나 System.in.read나 둘 중 하나 아무거나 써도 상관없음
		...
		do {
			...
			switch문은 조건식을 쓸수없고 산술식만 쓸 수 있기 때문에
			99점이나 91점을 10으로 나눴을때 몫이 9라는 사실을 통해
			...
			exitCode = System.in.read(); //read 문자를 아스키코드 값을 정수형태로 반환해줘서 위에 int로 exitCode가 잡힘
