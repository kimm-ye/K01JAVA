...
abstract class AbsClass1{
	...
	public static final int MAX_INT	 = Integer.MAX_VALUE; //21000000
...
class AbsClassChild1 extends AbsClass1{
	...
	@Override
	void instanceMethod() {
		System.out.println("필요없다면 오버라이딩 하지 않아도 상관없다."); //추상메서드를 포함하지 않아서 오버라이딩 굳이 안해도 됨
	}
}
...
추상클래스2 : 추상메서드를 포함한 클래스
	...
	그렇지 않으면 에러가 발생한다.
 */
abstract class AbsClass2{ //추상클래스
	//추상메서드는 실행부 즉 {} 중괄호를 쓰지 않는다.
	abstract void absMethod(int number); //추상메서드를 쓰려면 반드시 추상클래스 반드시 써야함
}

/*
위 클래스를 상속하면 부모가 가진 추상메서드를 그대로 상속받게 되므로
해당 클래스도 abstract로 선언해야 한다.
단, 이 경우 해당 클래스로도 객체를 생성할 수 없게 된다.
만약 객체를 생성해야 한다면 추상메서드를 자식에서 오버라이딩하면 된다.
 */
...
public class E01AbstractBasic {
	...
	public static void main(String[] args) {
		...
		//추상클래스는 객체를 생성할 수 없다.
		//AbsClass1 absClass1 = new AbsClass1(); //abstract는 객체 생성 불가하기 때문에 에러발생
