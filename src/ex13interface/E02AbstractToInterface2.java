...
interface IPersonalNumberStorage{
	...
	void addPersonalInfo(String jumin, String name); 
	String searchPersonalInfo(String jumin); 
}
...
class PersonalNumberStorageImpl implements IPersonalNumberStorage{
	...
	public PersonalNumberStorageImpl(int arrSize) {
		PersonalArr = new PersonalInfoVO[arrSize]; //객체배열을 생성하는데 arrSize로 만든다.
		numOfPerInfo = 0;
	}
	
	/*
	상속을 통해 추상메서드를 포함하게 되었으므로 반드시 아래와 같이 오버라이딩 해야한다.
	...
	public void addPersonalInfo(String juminNum, String name) {
		...
		numOfPerInfo++; //1명씩 추가하는거라서?
		//PersonalArr[numOfPerInfo++] = new PersonalInfoDTO(name, juminNum);로 작성해도 위에 두줄과 동일함
	}
	//주민번호를 인자로 객체 배열을 검색한 후 정보를 반환한다.
	@Override
	public String searchPersonalInfo(String jumin) {
		for(int i=0; i<numOfPerInfo; i++) {
			/*
			멤버변수를 private으로 선언했으므로 외부에서는 접근할 수 없다.
			이럴때 getter메서드를 통해 접근하여 값을 얻어올 수 있다.
			 */
			if(jumin.compareTo(PersonalArr[i].getJuminNum())==0) { //private으로 주민과 네임을 썻기 때문에 getter로 값을 가져와야함
				//getter를 통해 이름을 반환한다.
...
public class E02AbstractToInterface2 {
	...
	public static void main(String[] args) {
		...
		//PersonalNumberStorageImpl storage = new PersonalNumberStorageImpl(10); //ext로 써도 같은 패키지라 에러는 안남 
