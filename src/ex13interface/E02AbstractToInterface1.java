...
- 추상클래스는 대부분 추상메서드를 포함하고 있으므로
	이를 상속받는 클래스는 반드시 추상메서드를 오버라이딩 해야 한다.
...
abstract class PersonalNumberStorageAbs{
	...
	public abstract void addPersonalInfo(String jumin, String name); //실행부가 없어서 {} 안씀
	public abstract String searchPersonalInfo(String jumin); //실행부가 없어서 {} 안씀
}
...
class PersonalNumberStorageExt extends PersonalNumberStorageAbs{
	...
	public PersonalNumberStorageExt(int arrSize) {
		PersonalArr = new PersonalInfoDTO[arrSize]; //객체배열을 생성하는데 arrSize로 만든다.
		numOfPerInfo = 0;
	}
