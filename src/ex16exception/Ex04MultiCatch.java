...
이 경우 반드시 자식 예외클래스부터 catch블럭에 사용해야 한다.
Exception 클래스는 모든 예외를 catch하기 때문에 마지막 블럭에 사용해야 한다.
상속의 구조는 ctrl + t로 확인할 수 잇다.
 */
	...
	public static void main(String[] args) {
		...
		try {
			for(int i=0; i<array.length; i++) {
				...
				array[i] = Integer.parseInt(scanner.nextLine()); //NumberFormatException이거 쓸라고 
			}
			System.out.println("배열에 저장된 두 숫자나누기:" + array[0] / array[1]); //두번째 입력값에 0을 입력하면 예외발생
