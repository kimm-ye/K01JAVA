...
//class ChildProperty{
//	
//	int myBead; //child 구슬 갯수
//	
//	public ChildProperty(int b) { //보유갯수 초기화
//		myBead = b;
//	}
//	
//	public void showProperty() {//게임 전 구슬 보유 갯수
//		System.out.println(myBead);
//	}
//	
//	public void obtainBead(ChildProperty anotherChild, int addBead) { //다른아이 구슬 획득
//		
//		anotherChild.myBead -= addBead;
//		myBead += addBead;
//	}
//}
//
...
class ChildProperty{
	...
	void showProperty(){
		...
		System.out.printf("보유 구슬의 개수 %d%n", bead); //둘 중 하나 사용하면 됨
	}
	
	/*
	해당 멤버메서드의 호출의 주체가 되는 객체(여기선 child1)가 beadCnt만큼 획득하고
	매개변수로 전달되는 child객체는 구슬을 잃게 된다.
	 */
	void obtainBead(ChildProperty child, int beadCnt){
		if(!(beadCnt < bead && bead >0)) {
			System.out.println("구슬부족");
			return;
		}
		this.bead += beadCnt; //호출한 주체에게 더해준다
		child.bead -= beadCnt; //두번째 어린이가 가지고 있는 bead가 가지고 있는 거
