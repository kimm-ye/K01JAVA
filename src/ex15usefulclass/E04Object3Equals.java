...
class MyClass{
	...
	public String toString() {
		return String.valueOf(data); //valueOf = 정수를 문자열로 반환해줌
	}
	...
	public boolean equals(Object obj) {
		...
		if(obj instanceof MyClass) {
			/*
			매개변수가 MyClass 타입이 맞다면 비교를 위해 MyClass타입으로
			형변환(다운캐스팅)한다.
			 */
			...
			if(mc.data == this.data) {
				...
				return true; //값이 일치하면 true
			}
			else {
				System.out.println("MyClass-멤버 다름");
				return false; //값이 일치하지않으면 false
			}
		}
		else {
			/*
...
public class E04Object3Equals {
	...
	public static void main(String[] args) {
		...
		/*
		println()은 출력전 항상 객체의 toString()메서드를 자동으로 호출하므로
		아래 출력결과는 동일하다.
		 */
